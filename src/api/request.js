...
// const baseUrl = process.env.NODE_ENV === 'production' ? 'https://www.xxxxx.com/v1/' : 'http://localhost:4200/v1/' // 项目运行在开发环境还是生产环境，根据不同环境对应不同配置
const baseUrl = 'http://47.105.55.162/api/v1/' // 项目运行在开发环境还是生产环境，根据不同环境对应不同webpack配置
...
// 获取标签下所有的文章
export const getTagArticle = (tagId) => axios.get(`${baseUrl}tag?tagId=${tagId}`)
...
export const getAllArticle = () => axios.post(`${baseUrl}articles`)
